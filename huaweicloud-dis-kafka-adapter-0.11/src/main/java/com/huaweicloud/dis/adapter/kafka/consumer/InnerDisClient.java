...
class InnerDisClient extends DISClient implements ICoordinatorService,IAppService
{
    ...
    //    @Override
    public SplitPartitionResult splitShard(SplitPartitionRequest splitShardsRequest)
    {
        ...
        final String resourcePath = ResourcePathBuilder.standard()
                ...
                .withResource(new StreamResource(null, splitShardsRequest.getStreamName(), Constants.SPLITSHARD_RESOURCES))
                .build();
        request.setResourcePath(resourcePath);
        return request(splitShardsRequest, request, SplitPartitionResult.class);
    }

    //    @Override
    public MergePartitionsResult mergeShards(MergePartitionsRequest mergeShardsRequest)
    {
        Request<HttpRequest> request = new DefaultRequest<>(Constants.SERVICENAME);
        request.setHttpMethod(HttpMethodName.POST);
        ...
        final String resourcePath = ResourcePathBuilder.standard()
                .withProjectId(disConfig.getProjectId())
                .withResource(new StreamResource(null, mergeShardsRequest.getStreamName(), Constants.MERGESHARDS_RESOURCES))
                .build();
        request.setResourcePath(resourcePath);
        return request(mergeShardsRequest, request, MergePartitionsResult.class);
    }
