...
public class Coordinator {
    ...
    {
        List<TopicPartition> kafkaAssignment = new ArrayList<>();
        for(Map.Entry<String, List<Integer>> entry: assignment.entrySet())
        {
            for(Integer partition: entry.getValue())
            {
                kafkaAssignment.add(new TopicPartition(entry.getKey(),partition));
            }
        }
        Iterator<Map.Entry<TopicPartition, PartitionCursor>> iter = nextIterators.entrySet().iterator();
        while (iter.hasNext())
        {
            Map.Entry<TopicPartition, PartitionCursor> entry = iter.next();
            if(!kafkaAssignment.contains(entry.getKey()))
            {
                iter.remove();
            }
        }
        subscriptions.assignFromSubscribed(kafkaAssignment);
    }
    ...
    public void commitSync(Map<TopicPartition, OffsetAndMetadata> offsets)
    {
        ...
        for (Map.Entry<TopicPartition, OffsetAndMetadata> offset : offsets.entrySet())
        {
            ...
            commitCheckpointRequest.setStreamName(offset.getKey().topic());
    ...
    private void addPartition(Map<String ,List<Integer>> describeTopic,TopicPartition part)
    {
        if(describeTopic.get(part.topic()) == null)
        {
            describeTopic.putIfAbsent(part.topic(), new ArrayList<>());
        }
        describeTopic.get(part.topic()).add(part.partition());
    }
        ...
        for(Map.Entry<String,List<Integer>> entry: describeTopic.entrySet())
        {
            ...
            while(index < parts.size())
            {
                ...
                for(PartitionResult partitionResult:describeStreamResult.getPartitions())
                {
                    ...
                    {
                        TopicPartition partition = new TopicPartition(entry.getKey(),parts.get(index));
    ...
    private void resetOffset(TopicPartition partition) {
        ...
        DescribeStreamRequest describeStreamRequest = new DescribeStreamRequest();
        describeStreamRequest.setStreamName(partition.topic());
    ...
    public Map<TopicPartition, OffsetAndMetadata> fetchCommittedOffsets(Set<TopicPartition> partitions) {
        ...
        for(TopicPartition partition: partitions)
        {
            ...
            getCheckpointRequest.setStreamName(partition.topic());
    ...
    public void seek(final TopicPartition partition, CountDownLatch countDownLatch)
    {
        ...
        getShardIteratorParam.setStreamName(partition.topic());
